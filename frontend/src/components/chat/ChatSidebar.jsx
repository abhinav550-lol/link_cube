// ─── ChatSidebar ──────────────────────────────────────────────────────────────
// Lists all user chats; handles new chat creation and chat deletion.
...
export default function ChatSidebar({ chats, activeChatId, onSelect, onNewChat, onDelete }) {
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        {chats.length === 0 && (
          <p className="text-xs text-muted text-center py-8 px-4">
            No chats yet. Start a new one to query your documents.
          </p>
        )}
        {chats.map((chat) => (
          <div
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md animate-slide-up">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <label className="block text-xs font-mono uppercase tracking-wide text-slate mb-1.5">
              Scope to files (optional — empty = all your files)
            </label>
            {loadingFiles ? (
              <p className="text-xs text-muted">Loading files…</p>
            ) : files.length === 0 ? (
              <p className="text-xs text-muted">No ready files. Upload and wait for ingestion.</p>
            ) : (
