// ─── Signup Page ──────────────────────────────────────────────────────────────
// Calls POST /api/user/register with { name, email, password, role }.
// Role is selected by the user here at registration time.
...
export default function Signup() {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', confirm: '', role: 'ngo' });
  ...
  const handleSubmit = async (e) => {
    ...
    // signup() calls POST /api/user/register
      ...
      <div className="hidden lg:flex w-1/2 bg-ink flex-col justify-between p-12 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-4xl font-display font-semibold text-white leading-tight mb-4">
            Join the movement<br />of change.
          </h1>
          <p className="text-silver text-sm leading-relaxed max-w-sm">
            Whether you're an NGO coordinating relief or a volunteer on the ground —
            LinkCube brings your work together.
          </p>
        </div>

        <div className="relative z-10 space-y-4">
          {[
            { icon: '🗺️', text: 'Track villages and their specific needs' },
            { icon: '👥', text: 'Find and assign skilled volunteers fast' },
            { icon: '📊', text: 'Upload reports and monitor progress' },
          ].map((f) => (
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md animate-slide-up">
          <div className="flex items-center gap-2 mb-8 lg:hidden">
            ...
            <span className="text-ink text-lg font-display font-semibold">Sewa</span>
          </div>

          <h2 className="text-2xl font-display font-semibold text-ink mb-1">Create an account</h2>
          <p className="text-sm text-muted mb-8">Get started with Sewa in minutes.</p>

          <form onSubmit={handleSubmit} className="space-y-5">
            {/* Role selector — sent to POST /api/user/register as `role` field */}
            <div>
              <p className="text-xs font-mono uppercase tracking-wide text-slate mb-2">I am registering as…</p>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { value: 'ngo', label: 'NGO', sub: 'Organisation account' },
                  { value: 'volunteer', label: 'Volunteer', sub: 'Individual account' },
                ].map((r) => (
                  <button
                    key={r.value}
                    type="button"
            ...
            <Input
              label={form.role === 'ngo' ? 'Organisation Name' : 'Full Name'}
              type="text"
              placeholder={form.role === 'ngo' ? 'Aashray Foundation' : 'Arjun Mehta'}
              value={form.name}
