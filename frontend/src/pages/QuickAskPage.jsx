// ─── QuickAskPage ─────────────────────────────────────────────────────────────
// Stateless one-shot RAG Q&A — POST /api/rag/ask
// No chat history. For ongoing sessions use the Chat page.
...
export default function QuickAskPage() {
  ...
  // Load user's ready files for scoping
  useEffect(() => {
    filesApi
      .getMyUploads()
  ...
  const handleAsk = async (e) => {
    ...
    setResult(null);
    try {
      const res = await ragService.ask(q, selectedFileIds);
      setResult(res);
    } catch (err) {
    ...
    <div className="space-y-6 animate-slide-up max-w-2xl">
      ...
      <div>
        ...
        <p className="text-sm text-muted mt-1">
          Instant one-shot Q&amp;A across your documents — no conversation history.
        ...
        <form onSubmit={handleAsk} className="space-y-4">
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder="e.g. What are the main risks in the flood-affected areas?"
              value={question}
